...
describe("VisualizarPublicacao", () => {
  ...
  it("Testa a renderização da foto - válida", async () => {
    ...
    // Aqui, garantimos que o token seja uma string ou um valor padrão
    const token = (await AsyncStorage.getItem("token")) || "";

    const { getByTestId } = render(<PublicacaoVisualizar item={mockPublicacao} token={token} />);
  ...
  it("Renderiza corretamente o componente com dados de publicação", async () => {
    ...
    // Garantir que o token seja passado, se necessário
    const token = (await AsyncStorage.getItem("token")) || "";

    const { getByText } = render(<PublicacaoVisualizar item={mockPublicacao} token={token} />);
    await waitFor(() => {
